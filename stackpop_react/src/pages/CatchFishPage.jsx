...
export const CatchFish = () => {
    ...
    const [species, setSpecies] = useState('Anchovy');
    ...
    const addFish = async () => {
        const newFish = {species, month, time, location, weather}
        ...
        if(response.status===201){
            alert("Successfully added the fish");
        }else{
            alert("Failed to add fish, status code = " + response.status)
        }
        navigate('/')
    };
    ...
    return (
        <div>
        <h2>Log New Fish Below</h2>
        <p>Weather should be in MM-DD-YY format</p>
        <table>
            <thead>
                <tr>
                    <th>Species</th>
